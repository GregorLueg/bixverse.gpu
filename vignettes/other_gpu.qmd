...
a Rust BLAS with strong SIMD support, and a generic GEMM on the GPU only beat it
on very large data sets. `bixverse-rs` now ships handrolled Gram kernels for
correlation and covariance that actually beat `faer`, and `bixverse.gpu` exposes
them as `rs_cor_gpu()` and `rs_cov_gpu()` for when you need your correlations
