...
also run on CPU via the flex framework by the Burn development team), so
a compatible GPU is beneficial but not strictly required.
...
embedding and the trained encoder (to note: the data is stored in the
Rust kernel and handed around via a pointer). As this is small data, we
will just use the CPU version for 100 epochs, more than enough here.
...
#>   k: 15 | min_dist: 0.500 | spread: 1.0 | knn: kmknn
```
...
As we are using Rust under the hood with external memory pointers, we
need to be able to also save and load the model. To that aim, the
package provides two helpers which deal with all of the memory stuff for
you and use the blazingly fast [qs2](https://github.com/qsbase/qs2)
under the hood. Let’s save the model first.

``` r

f_path_model <- file.path(tempdir(), "trained_model.qs2")

save_parametric_umap(pumap, f_path_model)

rm(pumap)
```

And now let’s load it again and predict again the test data.

``` r

pumap <- load_parametric_umap(f_path_model)

test_embedding_2 <- predict(pumap, newdata = test_data)

# let's test that the predictions are still behaving...
tinytest::expect_equivalent(current = test_embedding, target = test_embedding_2)
#> ----- PASSED      : <-->
#>  call| tinytest::expect_equivalent(current = test_embedding, target = test_embedding_2)
```
