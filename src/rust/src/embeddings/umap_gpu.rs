...
pub fn get_params_umap_graph<T>(r_list: List) -> Result<UmapGraphParams<T>>
where
    T: ManifoldsFloat,
{
...
fn get_params_umap_optim<T>(r_list: List, min_dist: f64, spread: f64) -> Result<UmapOptimParams<T>>
where
    T: ManifoldsFloat,
{
...
#[derive(Debug)]
pub struct InternalUmapParamsGpu<T> {
    ...
    pub randomised: bool,
    /// Which of the possible optimisers to use. One of `"sgd"`, `"adam"`,
    /// `"adam_parallel"` or `"adam_gpu"`
    pub optimiser: String,
    /// The UMAP optimisation parameters.
    pub param_optimiser: UmapOptimParams<T>,
}

impl<T> InternalUmapParamsGpu<T>
where
    T: ManifoldsFloat,
{
    ...
    pub fn from_r_list(r_list: List, min_dist: f64, spread: f64) -> Result<Self> {
...
/// Wrapper function into the GPU-accelerated UMAP implementation in
/// `manifolds-rs`
///
/// This function uses under the hood the GPU version UMAP implementation from
/// the `manifolds-rs` crate.
///
/// ### Params
///
    ...
    R: Runtime,
{
    ...
    let internal = InternalUmapParamsGpu::from_r_list(umap_params, min_dist, spread)?;
    ...
    let umap_params = UmapParamsGpu::new(
        n_dim,
        k,
        internal.optimiser,
        internal.knn_method,
    ...
    let res = umap_gpu::<T, WgpuRuntime>(
        data,
        pre_computed_knn,
        &umap_params,
        device.clone(),
